...
**463 asersi otomatis lolos** di sembilan berkas uji — 270 di `backend/tests/` (34+42+44+36+114), 193 di `pipeline/` (130+34+14+15). Ditambah **29 asersi audit peramban** di `frontend/scripts/audit-prd.mjs`, yang menuntut backend dan frontend hidup sehingga sengaja TIDAK dijumlahkan ke angka di atas.
...
| 🔴 **Kunci Map Services JUGA membuka data misi, dan ia ada di bundel publik** | Diuji 26 Agu 2026: kunci yang sama menjawab **200** di `server.mapid.io/web/competition/menugo` (kunci palsu 401, tanpa kunci 400 — jadi ia benar-benar mengotentikasi). Kunci itu hidup di `frontend/.env` sebagai `VITE_` dan ter-bundel ke `dist/assets/*.js`, sehingga siapa pun bisa menarik 691 titik survei mentah. Melanggar aturan keras #1 DAN #2 — **risiko diskualifikasi**. Asumsi lama "kunci Maps cuma menghitung pemakaian" terbukti salah | Basemap menuntut kunci ada di peramban, jadi memindahkannya ke backend saja mematikan peta. Tanyakan MAPID dulu (kunci khusus basemap / pembatasan referrer); kalau tidak ada, dua kunci terpisah atau proksi tile. Rinciannya di `docs/aturan-lomba.md` |
...
| **Pusat kawasan Harjamukti meleset 4,4 km dari stasiunnya** | Titik pilot `(-6,3706 · 106,8556)` dipakai bertiga oleh `demo_seed.py`, `s1_ingest.py`, dan `frontend/src/config.ts` — ketiganya cocok, jadi tidak ada uji konsistensi yang bisa menangkapnya. Stasiun LRT Harjamukti yang sebenarnya ada di `(-6,37389 · 106,89567)`, terverifikasi ke OSM 26 Agu 2026: **4.443 m**. Akibatnya seluruh heksagon kawasan itu mengukur D03/D04 ke titik yang bukan stasiun, dan penarikan OSM radius 2,6 km di sana tidak menemukan satu pun stasiun rel — nol, sementara lima kawasan lain 2–10. Nol itulah satu-satunya gejala yang pernah muncul | Memindahkannya berarti membangun ulang grid H3 kawasan itu, membuang 290 rute ORS + 3 isochrone-nya, dan mengubah setiap skor di sana. **Keputusan pemilik repo** — sama kelasnya dengan D04. Sebelum diputuskan, angka Harjamukti sebaiknya tidak dipakai sebagai contoh di presentasi |
...
| **basemap.mapid.io membatasi laju per-IP, dan gagalnya 401 bukan 429** | Menjalankan audit peramban berulang kali membuat SELURUH ubin dijawab `401 Authorization Required` dari nginx selama beberapa menit - dengan kunci maupun tanpa, di semua zoom. Terbaca persis seperti kunci dicabut, padahal pulih sendiri. Diuji sesudah pulih: 12/12 ubin berbeda menjawab 200 baik berkunci maupun tidak | Jangan menyimpulkan kuncinya bermasalah dari 401 di ubin - uji `style.json` juga, karena ia jalur yang berbeda dan tetap 200 saat ubinnya diblokir. Dan jangan menjalankan audit peramban berkali-kali berturut-turut |
