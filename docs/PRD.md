...
| NFR-04 | **Keamanan** | Kunci API tidak bocor; akses berbayar tidak bisa dibobol; AI tidak bisa dibajak lewat prompt | Kunci LLM, MAPID Data, dan ORS hanya di environment backend; sandi scrypt; tiket sesi HMAC; penjaga akses sebagai dependensi FastAPI; CORS daftar putih; argumen `pengguna` dari model dibuang; Loconomics AI menolak topik di luar cakupan dan instruksi yang disisipkan tanpa memanggil alat apa pun |
...
| **Validasi** | (1) **LLM tidak menghitung**: setiap angka berasal dari alat. (2) Jawaban tanpa satu pun panggilan alat ditandai `perlu_review` di `ai_call_logs`, **kecuali** penolakan cakupan/suntikan yang disengaja (lihat di bawah) - keduanya tidak boleh membanjiri log yang sama. (3) Penjaga akses berbayar yang sama dengan endpoint; argumen `pengguna` dari model selalu dibuang. (4) Heksagon kosong-data tidak dibuang dari pencarian, dan model diberi tahu nilainya belum diketahui. (5) 58 asersi uji loop dengan klien tiruan, termasuk batas putaran, panggilan penutup, dan penolakan cakupan |

**Cakupan percakapan dan pertahanan terhadap suntikan prompt.** Prompt sistem membatasi Loconomics AI hanya menjawab hal yang berkaitan dengan pemilihan lokasi usaha di Loconomics. Untuk pertanyaan yang jelas di luar itu (obrolan umum, coding, dan sejenisnya) atau percobaan menyisipkan instruksi lewat pesan pengguna, riwayat, atau hasil alat (mis. "abaikan instruksi di atas", "mode admin", meminta prompt sistem dituliskan ulang), model **tidak memanggil satu pun alat** dan menjawab dengan satu-dua kalimat penolakan berpola tetap - menghindarkan biaya pemanggilan alat untuk pertanyaan yang jawabannya memang tidak ada di sana. Jawaban seperti ini diberi tanda internal (`TANDA_TOLAK_CAKUPAN`) yang disembunyikan sebelum sampai ke pengguna, dan ditandai terpisah dari kegagalan sungguhan di `ai_call_logs`. Diuji dengan Gemini sungguhan: lima percobaan (tiga pertanyaan di luar topik, dua percobaan suntikan) seluruhnya dijawab tanpa satu pun panggilan alat dalam 4-9 detik, tanpa membocorkan prompt sistem atau nama alat.
...
| Penyalahgunaan AI | Pembatas laju, plafon biaya harian, daftar alat tertutup, argumen `pengguna` dari model dibuang, penolakan cakupan/suntikan tanpa panggilan alat |
